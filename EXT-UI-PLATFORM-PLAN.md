...
- [ ] **W2.2** `npm test` + `npm run build` green ✅ (local). **Still to do:** visual/behavior parity in a running preview (the `/resumes` page is auth-gated → needs the stack up) + confirm CI green (esp. the new `web-docker` image build), then **merge the web change → deploy.** This is the W2 deploy gate.
